...
// @ts-ignore
import ReflectAI from './icons/face.svg'
// @ts-ignore
import ReflectAIWhite from './icons/face.svg'
...
export default {
    ...
    ReflectAIIcon: ({mode, style}: {style?: ViewStyle, mode?: TColorMode}) => 
        <View style={[{width: 15, height: 15, justifyContent: 'center', alignItems: 'center'}, style]}>
            { (mode ?? store.getState().main.app_mode) == 'dark' ? <ReflectAIWhite /> : <ReflectAI /> }
        </View>,
    ExpandIcon: ({style, onPress}: {style?: ViewStyle; onPress?: ((event: GestureResponderEvent) => void)}) => 
        <View style={[{width: 15, height: 15, justifyContent: 'center', alignItems: 'center'}, style]} onTouchEnd={onPress}>
